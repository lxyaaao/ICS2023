...
int byteSwap(int x, int n, int m) {
  int move_n = (n << 3);
  int move_m = (m << 3);
  int by_n = 0xff << move_n;
  int by_m = 0xff << move_m;
  int tmp = x & (~(by_n | by_m));
  int byte_n = (x >> move_n) & (0xff);
  int byte_m = (x >> move_m) & (0xff);
  return (tmp | (byte_n << move_m) | (byte_m << move_n));
}
...
int subOK(int x, int y) {
  int s_same = (~((x ^ y) >> 31)) & 1;
  int s_x = (x >> 31) & 1;
  int s_sub = ((x + (~y) + 1) >> 31) & 1;
  return (s_same | (!(s_x ^ s_sub)));
}
...
{
  int tmp1 = x >> 3;
  int ans1 = (tmp1 << 2) + tmp1;
  int tmp2 = x & 7;
  int ans2 = ((tmp2 << 2) + tmp2 + ((x >> 31) & 7)) >> 3;
  return ans1 + ans2;
}
...
unsigned float_half(unsigned uf) {
  ...
  if(exp == 0xff) 
    return uf;
  if(exp > 1) 
    return s | (frac | (exp - 1) << 23);
  round = !((uf & 3) ^ 3);
  return s | (((uf + round) >> 1) & 0xffffff);
}
